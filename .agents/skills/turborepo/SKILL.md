...
## This Project — `frontend-workspace`

> Read this section first when working in the `frontend-workspace` monorepo.
> For generic Turborepo rules, continue to the sections below.

**Stack:** pnpm@10.27.0 · turbo@^2.7.6 · node>=20.19

### Registered Turbo Tasks (root `turbo.json`)

| Task          | `dependsOn`    | notes                       |
| ------------- | -------------- | --------------------------- |
| `build`       | `^build`       | packages build first → apps |
| `lint`        | `^lint`        | parallel, no cache dep      |
| `check-types` | `^check-types` | parallel, no cache dep      |
| `dev`         | `^build`       | persistent, `cache: false`  |

> [!IMPORTANT]
> **`test` and `storybook` are NOT turbo tasks.** Run them directly:
>
> ```bash
> pnpm --filter @repo/ui storybook        # Storybook dev (port 6006)
> pnpm --filter @repo/ui build-storybook  # Storybook static build
> ```
>
> The type check script is **`check-types`** — never `type-check`.

### Packages and Their `--filter` Selectors

| Package                | `--filter` selector              |
| ---------------------- | -------------------------------- |
| Shared component lib   | `@repo/ui`                       |
| Shared Tailwind config | `@repo/config`                   |
| Shared tsconfigs       | `@repo/typescript-config`        |
| Shared ESLint rules    | `@repo/eslint-config`            |
| Shared types           | `@repo/interface`                |
| Admin portal app       | `admin-portal` (no @repo/ scope) |
| Web app                | `web` (no @repo/ scope)          |

### Common Commands in This Project

```bash
# Run a specific task on one package
pnpm --filter @repo/ui build
pnpm --filter @repo/ui check-types
pnpm --filter admin-portal build

# Run a turbo task across all affected packages
pnpm run build          # delegates to turbo run build (all)
pnpm run check-types    # delegates to turbo run check-types (all)
pnpm run lint           # delegates to turbo run lint (all)

# Add a dep to a specific package
pnpm --filter @repo/ui add <package>
pnpm --filter admin-portal add -D <package>

# Install workspace root dev tools
